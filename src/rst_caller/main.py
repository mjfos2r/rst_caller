...
def amplify_and_cut(input_fa_file, quiet=False):
    ...
    with open(input_fa_file, "r") as f:
        proc_out = subprocess.run(
            ["seqkit", "amplicon", "-F", fwd, "-R", rev, "-m", "2", "-M", "-I"],
            stdin=f,
            text=True,
            capture_output=True,
        )

        amplicon = [rec for rec in SeqIO.parse(io.StringIO(proc_out.stdout), "fasta")][
            0
        ]
        # run the digestion but toss out the MseI fragments below 100.
        HinfI_fragments = sorted(
            {
                len(frag)
                for frag in Restriction.HinfI.catalyze(amplicon.seq, linear=True)
            }
        )
        MseI_fragments = sorted(
            {
                len(frag)
        ...
        # given that the largest HinfI fragments is typically 1037 instead of 1078 let's give it some wiggle room to account for indels.
        # and MseI is usually pretty close. let's use a semi-arbitrary cutoff of 17bp since Jones et al. 2006 use 18bp as their
        # minimum length to distinguish bands. (for ospC typing via RFLP)
